...
async def get_start(message: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=settings.bots.admin_id, text=f'Заявка от {message.from_user.full_name}', reply_markup=kb.get_start)
...
async def start(message: Message, state: FSMContext):
    ...
    await message.bot.send_message(chat_id=message.from_user.id, text="Вы начали чат с администратором. Отправьте своё сообщение.")
    await state.set_state(Register.start_dialog)
...
async def handle_message(message: Message, state: FSMContext):
    if message.from_user.id in user_to_admin:
        await bot.send_message(settings.bots.admin_id, f"Пользователь с ID {message.from_user.id} пишет: {message.text}")
...
async def handle_admin_message(message: Message, state: FSMContext):
    for user_id, admin in user_to_admin.items():
        if admin == settings.bots.admin_id:
            await bot.send_message(user_id, f"Администратор отвечает: {message.text}")
    await state.set_state(Register.start_dialog)